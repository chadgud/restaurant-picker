...
const Decide = () => {
    ...
    let fourSquareData = state.results;

    useEffect(() => {
        if (state.fromSwitch) {
            setPerson1Data(state.person1Data);
            setPerson('person2');
            setPlaceIndex(0);
            state.fromSwitch = false;
        }
        ...
        if (person1Data.length === fourSquareData.length && person2Data.length === fourSquareData.length) {
            navigate('/recorded', {
                state: {
                    ...
                    restaurants: fourSquareData
                }
            });
        }

        if (person1Data.length === fourSquareData.length && person === 'person1') {
            navigate('/switch', {
                state: {
    ...
    const handleThumb = (isThumbsUp) => {
        if (person === 'person1') {
            setPerson1Data((person1Data) => [...person1Data, isThumbsUp ? 1 : 0]);
        } else {
            setPerson2Data((person2Data) => [...person2Data, isThumbsUp ? 1 : 0]);
        }

        if (placeIndex < fourSquareData.length - 1) {
    ...
    return <div className='row h-100 d-flex flex-column justify-content-center'>
        <div className="col-md-8 col-lg-6 offset-md-2 offset-lg-3">
            <div className="card">
                ...
                <div className='card-body'>
                    ...
                    <p className='my-0'>Price: {restaurant.price ? '' : 'No Price Information Available'} <strong>{'$'.repeat(restaurant.price)}</strong><strong className='text-body text-opacity-25'>{'$'.repeat(4 - restaurant.price)}</strong></p>
                    <button className="btn btn-danger w-50 mt-2" onClick={() => handleThumb(false)}>
                    ...
                    <button className="btn btn-success w-50 mt-2" onClick={() => handleThumb(true)}>
